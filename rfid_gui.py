"""
rfid_gui.py  —  Windows RFID Yönetim Arayüzü
=============================================
İki sekme:
  1. Kart Tanıtma  — öğrenciye RFID kartı ata
  2. Yoklama       — ders sırasında kartları Firebase'e gönder
...
# rfid_to_firebase.py ile aynı klasörde
...
def _ensure_firebase() -> bool:
    ...
    try:
        ...
        if not key.exists():
            _fb_error = f"serviceAccountKey.json bulunamadı:\n{key}"
            return False
...
# ─── AntennaClient (rfid_to_firebase.py'den kopyalandı) ──────────────────────
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("RFID Yönetim Sistemi")
        self.geometry("820x600")
        self.minsize(700, 500)
        self.configure(bg=C_BG)
        self.resizable(True, True)

        # Anten bağlantı ayarları
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="📡  RFID Yönetim Sistemi",
                 font=("Segoe UI", 14, "bold"), fg="white", bg=C_NEU).pack(
                     side="left", padx=20, pady=14)

        # Firebase durum etiketi
        self._fb_lbl = tk.Label(header, text="⏳ Firebase bağlanıyor...",
        ...
        self._nb.add(self._enroll_tab, text="  🪪  Kart Tanıtma  ")
    ...
    def _poll_connection(self):
        if self._client and self._client.connected:
            self._conn_lbl.config(text="🟢 Anten bağlı", fg=C_GREEN)
        else:
            self._conn_lbl.config(text="🔴 Anten bağlanamadı", fg=C_RED)
    ...
    def _init_firebase_async(self):
        def run():
            ...
            self.after(0, lambda: self._fb_lbl.config(
                text="✅ Firebase bağlı" if ok else f"❌ Firebase hatası",
...
class EnrollTab(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(left, text="RFID Kart Atanmamış Öğrenciler",
        ...
        tk.Label(inner, text="Seçili Öğrenci",
                 font=("Segoe UI", 10, "bold"), fg=C_GRAY, bg=C_CARD).pack(anchor="w")
        self._sel_lbl = tk.Label(inner, text="— seçim yok —",
        ...
        self._enroll_btn = tk.Button(inner, text="🪪  Kartı Tara",
    ...
    def _fill_list(self):
        ...
        if not self._filtered:
            self._listbox.insert("end", "  Kayıt bulunamadı")

    def _filter(self):
        self._fill_list()
        self._selected_idx = None
        self._sel_lbl.config(text="— seçim yok —")
    ...
    def _start_enroll(self):
        ...
        if client is None:
            self._set_status("❌ Anten IP girilmemiş.", C_RED); return

        self._enrolling = True
        self._enroll_btn.config(state="disabled", text="⏳ İşleniyor...")
        self._progress.pack(fill="x", pady=(8,0))
        self._progress.start(12)
        self._set_status(f"⏳ Anten bağlanıyor ve buffer temizleniyor...", C_ORANGE)

        def run():
            try:
                time.sleep(1.0)
                client.flush()
                self.after(0, lambda: self._set_status("📦 Arka plan taranıyor (3 sn)...", C_ORANGE))
                background = client.sample(3.0)
                self.after(0, lambda: self._set_status(
                    f"🪪 Lütfen '{ad}' kartını antene tutun...", C_NEU))
                client.flush()
                time.sleep(0.5)
                ...
                if not new_uids:
                    self.after(0, lambda: self._set_status(
                        "❌ Kart okunamadı. Kart aralığa uygun mu?\nTekrar deneyin.", C_RED))
                    return
                ...
                self._selected_idx = None
                self.after(0, self._fill_list)
                self.after(0, lambda: self._sel_lbl.config(text="— seçim yok —"))
...
class ListenTab(tk.Frame):
    ...
    def _build(self):
        ...
        self._listen_btn = tk.Button(inner, text="▶  Başlat",
    ...
    def _start(self):
        if not _ensure_firebase():
            messagebox.showerror("Hata", f"Firebase bağlantısı yok:\n{_fb_error}")
            return
        client = self._app.get_client()
        if client is None:
            messagebox.showerror("Hata", "Anten IP girilmemiş.")
            return
        self._listening = True
        self._stop_event.clear()
        self._last_pushed.clear()
        self._listen_btn.config(text="⏹  Durdur", bg=C_RED)
        self._log_append("─── Dinleme başlatıldı ───", "info")
    ...
    def _stop(self):
        ...
        self._listen_btn.config(text="▶  Başlat", bg=C_GREEN)
    ...
    def _push_uid(self, uid: str, ts: float):
        try:
            ...
            self.after(0, lambda: self._log_append(f"❌ Firebase hatası: {e}", "error"))
    ...
    def _log_warn(self, uid: str):
        zaman = time.strftime("%H:%M:%S")
        self._log_append(f"[{zaman}]  ", "ts", newline=False)
        self._log_append(f"⚠️  Kayıtsız kart: {uid}\n", "warn")
